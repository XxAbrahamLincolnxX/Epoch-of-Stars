...
export function GameCanvas() {
  ...
  }, [gameState.position, gameState.size, absorbParticle, checkEvolution, updateGameState]);
...
function drawStars(ctx: CanvasRenderingContext2D, width: number, height: number) {
  ...
  // Reduced to 20 stars for cleaner background
  for (let i = 0; i < 20; i++) {
...
function drawPlayer(ctx: CanvasRenderingContext2D, gameState: any) {
  ...
  // Draw outer glow ring for visibility
  ...
  // Draw visible ring outline (brighter and thicker)
...
function drawParticle(ctx: CanvasRenderingContext2D, particle: any) {
  // Add glow to particles for better visibility
  ctx.save();
...
function getStageColor(stage: string, alpha: number): string {
  const colors: Record<string, string> = {
    hydrogen: `rgba(135, 206, 250, ${alpha})`, // Light blue
    molecule: `rgba(173, 216, 230, ${alpha})`, // Light blue
    dust_grain: `rgba(210, 180, 140, ${alpha})`, // Tan
    planetoid: `rgba(160, 82, 45, ${alpha})`, // Saddle brown
    planet: `rgba(65, 105, 225, ${alpha})`, // Royal blue
    brown_dwarf: `rgba(165, 42, 42, ${alpha})`, // Brown
    red_dwarf: `rgba(255, 69, 0, ${alpha})`, // Red orange
    main_sequence: `rgba(255, 255, 0, ${alpha})`, // Yellow
    giant_star: `rgba(255, 140, 0, ${alpha})`, // Dark orange
    supergiant: `rgba(255, 0, 0, ${alpha})`, // Red
    black_hole: `rgba(148, 0, 211, ${alpha})`, // Dark violet
    galaxy: `rgba(255, 215, 0, ${alpha})` // Gold
  };
...
function getParticleColor(type: string): string {
  const colors: Record<string, string> = {
    hydrogen: '#87CEEB', // Sky blue
    helium: '#FFE4B5', // Moccasin
    dust: '#CD853F', // Peru
    debris: '#A9A9A9' // Dark gray
  };
  return colors[type] || '#87CEEB';
}

function generateParticles(count: number): any[] {
  const particles = [];
  for (let i = 0; i < count; i++) {
    particles.push(createRandomParticle(800, 600));
  }
  return particles;
}

function createRandomParticle(width: number, height: number): any {
  const types = ['hydrogen', 'helium', 'dust', 'debris'];
  const weights = [70, 20, 8, 2]; // Hydrogen is most common
  ...
  return {
    ...
    velocity: {
      x: (Math.random() - 0.5) * 0.8, // Even slower movement
...
function getSizeForType(type: string): number {
  const sizes: Record<string, number> = {
    hydrogen: 4,  // Smaller but still visible
    helium: 5,    
    dust: 7,      
    debris: 9     
  };
